...
## default port prefixes for uptickd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
DATA_DIR=$(mktemp -d -t uptick-datadir.XXXXX)
...
init_func() {
    "$PWD"/build/uptickd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/uptickd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/uptickd add-genesis-account \
        "$("$PWD"/build/uptickd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000auptick,1000000000000000000stake \
        --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/uptickd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/uptickd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/uptickd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/uptickd start --pruning=nothing --rpc.unsafe \
    ...
    UPTICK_PID=$!
    echo "started uptick node, pid=$UPTICK_PID"
    # add PID to array
    arr+=("$UPTICK_PID")
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test uptick node $HOST_RPC ..."
...
stop_func() {
    UPTICK_PID=$i
    echo "shutting down node, pid=$UPTICK_PID ..."

    # Shutdown uptick node
    kill -9 "$UPTICK_PID"
    wait "$UPTICK_PID"
